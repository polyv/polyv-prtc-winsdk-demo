...
public:
	...
	virtual int enableExtendVideoSource(bool enable, PRTCExtendVideoCaptureSource* source) = 0;
	virtual int enableExtendAudiocapture(bool enable, PRTCExtendAudioCaptureSource* audiocapture)  = 0;
	...
	virtual int StartPushCDN(const char* url, tPRTCTranscodeConfig *config) = 0;
    virtual int UpdatePushCDN(const char* url, tPRTCTranscodeConfig *config) = 0;
	virtual int StopPushCDN(const char* url) = 0;

	virtual int SetStreamRole(int role) = 0;
    virtual void SetVideoProfile(ePRTCVideoProfile profile) = 0;
