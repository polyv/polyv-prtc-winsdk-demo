...
BOOL CLoginDlg::OnInitDialog()
{
	...
	m_userid.SetWindowTextW(Ansi2WChar(PRTCConfig::getInstance()->getUserId().data()).data());
	m_roomid.SetWindowTextW(Ansi2WChar(PRTCConfig::getInstance()->getRoomId().data()).data());
	return TRUE;
}
...
void CLoginDlg::OnBnClickedButtonLogin()
{
	...
	PRTCConfig::getInstance()->setUserId(userid);
	PRTCConfig::getInstance()->setRoomId(roomid);

	PRTCConfig::getInstance()->setMuteCamBeforeJoin(m_mutecambefore.GetCheck());
	PRTCConfig::getInstance()->setMuteMicBeforeJoin(m_mutemicbefore.GetCheck());
